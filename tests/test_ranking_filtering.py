...
forks_main_data = [
    ...
    {"position": 1, "prev_uuid": _uuid("OTHER_PARENT_UUID"), "uuid": H1D_OTHER_PARENT, "fork_uuid": _uuid("fork_1_OTHER_H1D")},
...
ratings_pos1_data = [
    {"uuid": _uuid("vote1"), "voter": _uuid("voter1"), "winner": H1A, "loser": H1B}, # H1A vence H1B
    {"uuid": _uuid("vote2"), "voter": _uuid("voter2"), "winner": H1A, "loser": H1B}, # H1A vence H1B
    {"uuid": _uuid("vote3"), "voter": _uuid("voter3"), "winner": H1B, "loser": H1A}, # H1B vence H1A
...
def create_csv(data: list[dict], path: Path):
    if data:
        ...
        if data == [] and path.name.startswith("position_"): # Arquivo de ratings
             ...
             pd.DataFrame(columns=["position", "prev_uuid", "uuid", "fork_uuid"]).to_csv(path, index=False)
...
def test_get_ranking_empty_ratings_files(temp_data_dir):
    ...
    """
    forking_dir, ratings_dir = temp_data_dir
    create_csv(forks_main_data, forking_dir / "forks_main.csv") # H1A, H1B, H1C são herdeiros de H0_ROOT para pos 1

    # Criar arquivo position_001.csv vazio (só cabeçalho)
    pd.DataFrame(columns=["uuid", "voter", "winner", "loser"]).to_csv(ratings_dir / "position_001.csv", index=False)
...
def test_get_ranking_malformed_forking_csv(temp_data_dir):
    ...
    # CSV malformado
    (forking_dir / "bad_forks.csv").write_text("position,prev_uuid\ninvalid,row,with,too,many,columns")
