...
from hronir_encyclopedia import cli, storage, ratings

# Helper to create minimal hrönir files for testing
def create_mock_hronir(library_path: Path, text_content: str, uuid_override: str | None = None) -> str:
...
def setup_test_environment(tmp_path: Path) -> dict[str, Path]:
    ...
    # --- Create Hrönirs ---
    # Position 0
    hronir_seed = create_mock_hronir(library_dir, "Seed content", "00000000-seed-0000-0000-000000000000") # Placeholder for actual seed if needed by forks
    hronir_A_content = "Content for Hrönir A, successor of Fork A"
    hronir_A_suc = create_mock_hronir(library_dir, hronir_A_content, "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa")

    hronir_B_content = "Content for Hrönir B, successor of Fork B"
    hronir_B_suc = create_mock_hronir(library_dir, hronir_B_content, "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb")
    ...
    hronir_C_suc = create_mock_hronir(library_dir, hronir_C_content, "cccccccc-cccc-cccc-cccc-cccccccccccc")
    hronir_D_content = "Content for Hrönir D, successor of Fork D (child of A)"
    hronir_D_suc = create_mock_hronir(library_dir, hronir_D_content, "dddddddd-dddd-dddd-dddd-dddddddddddd")
    ...
    hronir_E_suc = create_mock_hronir(library_dir, hronir_E_content, "eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee")
    hronir_F_content = "Content for Hrönir F, successor of Fork F (child of B)"
    hronir_F_suc = create_mock_hronir(library_dir, hronir_F_content, "ffffffff-ffff-ffff-ffff-ffffffffffff")

    # --- Define Forks ---
    # Position 0 Forks
    fork_A_uuid = storage.compute_forking_uuid(position=0, prev_uuid="", cur_uuid=hronir_A_suc) # Fork A leads to hrönir_A_suc
    fork_B_uuid = storage.compute_forking_uuid(position=0, prev_uuid="", cur_uuid=hronir_B_suc) # Fork B leads to hrönir_B_suc

    # Position 1 Forks (Children of A's successor)
    fork_C_uuid = storage.compute_forking_uuid(position=1, prev_uuid=hronir_A_suc, cur_uuid=hronir_C_suc)
    fork_D_uuid = storage.compute_forking_uuid(position=1, prev_uuid=hronir_A_suc, cur_uuid=hronir_D_suc)

    # Position 1 Forks (Children of B's successor)
    fork_E_uuid = storage.compute_forking_uuid(position=1, prev_uuid=hronir_B_suc, cur_uuid=hronir_E_suc)
    fork_F_uuid = storage.compute_forking_uuid(position=1, prev_uuid=hronir_B_suc, cur_uuid=hronir_F_suc)
    ...
    initial_canonical_path_content = {
        ...
        "path": {
            "0": {"fork_uuid": fork_A_uuid, "hrönir_uuid": hronir_A_suc}
        }
    }
    ...
    dummy_voter_hronir_uuid = create_mock_hronir(library_dir, dummy_voter_hronir_content, "dddddddd-voter-dummy-dddddddddddd")
    voter_for_initial_ratings_fork_uuid = storage.compute_forking_uuid(position=99, prev_uuid="", cur_uuid=dummy_voter_hronir_uuid)
    # Add this dummy fork to a separate CSV to ensure it's found by storage.forking_path_exists
    create_fork_csv(fork_dir, "dummy_forks.csv", [{"position":99, "prev_uuid":"", "uuid":dummy_voter_hronir_uuid, "fork_uuid":voter_for_initial_ratings_fork_uuid}])
    ...
    ratings_pos0_data = [
        {"voter": voter_for_initial_ratings_fork_uuid, "winner": hronir_A_suc, "loser": hronir_B_suc, "elo_winner_pre": 1500, "elo_loser_pre": 1500, "elo_winner_post": 1516, "elo_loser_post": 1484}
    ]
    ...
    voter_pos1_A_children_fork_uuid = storage.compute_forking_uuid(position=98, prev_uuid="", cur_uuid=dummy_voter_hronir_uuid) # Re-use dummy hronir for new fork
    create_fork_csv(fork_dir, "dummy_forks_pos1A.csv", [{"position":98, "prev_uuid":"", "uuid":dummy_voter_hronir_uuid, "fork_uuid":voter_pos1_A_children_fork_uuid}])

    ratings_pos1_children_A_data = [
         # Make C slightly ahead of D
        {"voter": voter_pos1_A_children_fork_uuid, "winner": hronir_C_suc, "loser": hronir_D_suc, "elo_winner_pre": 1500, "elo_loser_pre": 1500, "elo_winner_post": 1516, "elo_loser_post": 1484}
    ]
    ...
    # Voter for these can be another dummy fork.
    voter_pos1_B_children_fork_uuid = storage.compute_forking_uuid(position=97, prev_uuid="", cur_uuid=dummy_voter_hronir_uuid) # Re-use dummy hronir for new fork
    create_fork_csv(fork_dir, "dummy_forks_pos1B.csv", [{"position":97, "prev_uuid":"", "uuid":dummy_voter_hronir_uuid, "fork_uuid":voter_pos1_B_children_fork_uuid}])
    ratings_pos1_children_B_data = [
        # Make E slightly ahead of F
        {"voter": voter_pos1_B_children_fork_uuid, "winner": hronir_E_suc, "loser": hronir_F_suc, "elo_winner_pre": 1500, "elo_loser_pre": 1500, "elo_winner_post": 1516, "elo_loser_post": 1484}
    ]
    ...
    return {
        ...
        "fork_A_uuid": fork_A_uuid, "hronir_A_suc": hronir_A_suc,
        "fork_B_uuid": fork_B_uuid, "hronir_B_suc": hronir_B_suc,
        "fork_C_uuid": fork_C_uuid, "hronir_C_suc": hronir_C_suc,
        "fork_D_uuid": fork_D_uuid, "hronir_D_suc": hronir_D_suc,
        "fork_E_uuid": fork_E_uuid, "hronir_E_suc": hronir_E_suc,
        "fork_F_uuid": fork_F_uuid, "hronir_F_suc": hronir_F_suc,
...
def test_system_dynamics_cascade(setup_test_environment):
    ...
    result_get_duel_pos0 = runner.invoke(
        ...
        [
            "get-duel",
            "--position", "0",
            "--ratings-dir", str(env["ratings_dir"]),
            "--forking-path-dir", str(env["fork_dir"]),
            "--canonical-path-file", str(env["canonical_path_file"]),
        ],
    ...
    assert {duel_info_pos0["duel_pair"]["fork_A"], duel_info_pos0["duel_pair"]["fork_B"]} == {env["fork_A_uuid"], env["fork_B_uuid"]}
    ...
    result_vote = runner.invoke(
        ...
        [
            "vote",
            "--position", "0",
            "--voter-fork-uuid", env["voter_for_decisive_vote_fork_uuid"],
            "--winner-fork-uuid", vote_winner_fork, # Fork B wins
            "--loser-fork-uuid", vote_loser_fork,   # Fork A loses
            "--ratings-dir", str(env["ratings_dir"]),
            "--forking-path-dir", str(env["fork_dir"]),
            "--canonical-path-file", str(env["canonical_path_file"]),
        ],
    ...
    assert vote_output["recorded_duel_hrönirs"]["winner"] == env["hronir_B_suc"] # Successor of Fork B
    assert vote_output["recorded_duel_hrönirs"]["loser"] == env["hronir_A_suc"]   # Successor of Fork A
    ...
    with open(env["canonical_path_file"], "r") as f:
        updated_canonical_data = json.load(f)

    assert "0" in updated_canonical_data["path"], "Position 0 should be in the updated canonical path."
    ...
    voter_pos1_B_children_fork_uuid = storage.compute_forking_uuid(position=96, prev_uuid="", cur_uuid=env["hronir_A_suc"]) # Just need a valid fork UUID
    create_fork_csv(env["fork_dir"], "dummy_forks_pos1B_extra.csv", [{"position":96, "prev_uuid":"", "uuid":env["hronir_A_suc"], "fork_uuid":voter_pos1_B_children_fork_uuid}])

    ratings_pos1_children_B_data = [
        {"voter": voter_pos1_B_children_fork_uuid, "winner": env["hronir_E_suc"], "loser": env["hronir_F_suc"], "elo_winner_pre": 1500, "elo_loser_pre": 1500, "elo_winner_post": 1516, "elo_loser_post": 1484}
    ]
    ...
    assert result_get_duel_pos1.exit_code == 0, f"get-duel for pos 1 failed. Output: {result_get_duel_pos1.stdout}"
    ...
    returned_duel_forks = {duel_info_pos1["duel_pair"]["fork_A"], duel_info_pos1["duel_pair"]["fork_B"]}
