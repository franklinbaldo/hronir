...
| SC.6  | Canonization by Consensus      | A versão canônica do livro é determinada periodicamente através do comando `consolidate_book`. Para cada posição, o hrönir com a maior pontuação Elo, resultante dos votos em duelos curados, é copiado para `book/` e `book/book_index.json`. |
| SC.7  | Curadoria de Duelos por Relevância Informativa | Os duelos são curados pelo sistema para maximizar a informação obtida por cada voto. A prioridade é dada a "Duelos de Calibração" (um hrönir novo contra o campeão da posição). Na ausência destes, o sistema apresenta o "Duelo de Máxima Entropia", o confronto com a maior incerteza matemática (tipicamente, hrönirs com Elo próximo). Votos só são aceitos para o duelo curado ativo para a posição, obtido via `get-duel`. |
