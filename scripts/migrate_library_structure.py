...
def migrate():
    ...
    for meta_path in all_metadata_files:
        try:
            ...
            if old_dir.name == LIBRARY_BASE.name or len(old_dir.name) > 1: # se o pai for a própria library_base ou um UUID
                if meta_path.parent.parent == LIBRARY_BASE: # Confirma que é um diretório de UUID diretamente sob LIBRARY_BASE
    ...
    unique_old_dirs = sorted(list(set(migrated_hronirs_old_paths)), key=lambda p: len(p.parts), reverse=True)
        ...
        try:
            ...
            if old_hronir_path_root.exists() and not any(old_hronir_path_root.iterdir()):
                ...
                if old_hronir_path_root.exists():
                    print(f"Skipping removal of non-empty or non-existent old hrönir directory: {old_hronir_path_root}")
            ...
            while current_path_to_clean != LIBRARY_BASE and current_path_to_clean.exists():
                if not any(current_path_to_clean.iterdir()): # Checa se o diretório está vazio
                    ...
                    print(f"Parent directory {current_path_to_clean} is not empty. Stopping cleanup for this path.")
                    break
        except OSError as e:
