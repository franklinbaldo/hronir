...
def is_fork_consumed(fork_uuid: str) -> Optional[str]:
...
def create_session(fork_n_uuid: str, position_n: int, forking_path_dir: Path, ratings_dir: Path, canonical_path_file: Path) -> Dict[str, Any]:
    """
    ...
    for p_idx in range(position_n - 1, -1, -1):
        ...
        if p_idx > 0:
            ...
            canonical_fork_info_prev_pos = storage.get_canonical_fork_info(p_idx - 1, canonical_path_file)
        ...
        if duel_info and "duel_pair" in duel_info and duel_info["duel_pair"].get("fork_A") and duel_info["duel_pair"].get("fork_B"):
            dossier_duels[str(p_idx)] = {
                ...
                "entropy": duel_info.get("entropy", 0.0) # Store entropy for informational purposes
            }
