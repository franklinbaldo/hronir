...
def _create_path_for_hronir(
    ...
    if predecessor_uuid:
        ...
        if not dm.hrönir_exists(predecessor_uuid):
            typer.secho(
                f"Error: Predecessor hrönir {predecessor_uuid} not found.", fg=typer.colors.RED
            )
            raise typer.Exit(1)
        ...
        if position is None:
            ...
            if parent_path:
                ...
                typer.secho(
                    f"Error: Could not determine position from predecessor {predecessor_uuid}. Please specify --position.",
                    fg=typer.colors.RED,
                )
                raise typer.Exit(1)
    else:
    ...
    dm.save_all_data()
    typer.echo(
        f"Created path {path_uuid_obj} at position {position} linking to predecessor {predecessor_uuid or 'None'}."
    )
...
def store_command(
    ...
    predecessor: Annotated[
        str | None, typer.Option("--predecessor", help="UUID of the predecessor hrönir.")
    ] = None,
    position: Annotated[
        int | None,
        typer.Option("--position", help="Explicit position (optional, inferred from predecessor)."),
    ] = None,
):
...
def synthesize_command(
    ...
    position: Annotated[
        int | None, typer.Option("--position", help="Explicit position (optional).")
    ] = None,
