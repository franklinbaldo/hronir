import dataclasses
import datetime
import gzip  # Replaced zstandard
import hashlib
import json
import logging
import os
...
# import zstandard # Removed

# Placeholder for DUCKDB_SCHEMA if it's essential and not easily importable
# from ..scripts.migrate_to_duckdb import DUCKDB_SCHEMA # This would be an import if scripts was a module
DUCKDB_SCHEMA = {
    "paths": "CREATE TABLE IF NOT EXISTS paths(path_uuid TEXT PRIMARY KEY, position INTEGER, prev_uuid TEXT, uuid TEXT, status TEXT, mandate_id TEXT);",
    "votes": "CREATE TABLE IF NOT EXISTS votes(uuid TEXT PRIMARY KEY, position INTEGER, voter TEXT, winner TEXT, loser TEXT);",
    "transactions": "CREATE TABLE IF NOT EXISTS transactions(uuid TEXT PRIMARY KEY, data TEXT);",
}  # Minimal placeholder
...
class ShardInfo:
    ...
    tables: list[str] | None = None  # List of tables in this shard (if sharded by table)
...
class SnapshotManifest:
    ...
    sequence: int | None = None  # Monotonic counter, assigned later by ConflictDetection
    prev_sequence: int | None = None  # Parent snapshot sequence, assigned later
    network_uuid: str | None = None  # Network identifier, assigned later
    created_at: datetime.datetime = dataclasses.field(
        default_factory=lambda: datetime.datetime.now(datetime.timezone.utc)
    )
    git_commit: str | None = None  # Git commit hash of the code version
    pgp_signature: str | None = None  # PGP signature of the manifest, assigned later
    merge_script: str | None = None  # SQL script or instructions to merge shards
    snapshot_tool_version: str = "0.1.0"  # Version of the sharding/snapshot tool
    compression_algorithm: str = "gzip"  # Changed from zstd
...
def compress_gzip(input_path: Path, output_path: Path, compresslevel: int = 9) -> None:
    """Compresses a file using Gzip."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with (
        open(input_path, "rb") as f_in,
        gzip.open(output_path, "wb", compresslevel=compresslevel) as f_out,
    ):
        shutil.copyfileobj(f_in, f_out)
    logging.info(f"Compressed {input_path} to {output_path} using Gzip (Level {compresslevel})")
...
def decompress_gzip(input_path: Path, output_path: Path) -> None:
    ...
    with gzip.open(input_path, "rb") as f_in, open(output_path, "wb") as f_out:
        shutil.copyfileobj(f_in, f_out)
    logging.info(f"Decompressed {input_path} to {output_path} using Gzip")
...
class ShardingManager:
    ...
    def create_sharded_snapshot(
        self, duckdb_path: Path, output_dir: Path, network_uuid: str, git_commit: str | None = None
    ) -> SnapshotManifest:
        """
        ...
        if compressed_estimate <= self.MAX_SHARD_SIZE_BYTES:
            ...
            compressed_file_name = f"{snapshot_filename_base}.db.gz"  # Changed extension
            compressed_file_path = output_dir / compressed_file_name

            compress_gzip(duckdb_path, compressed_file_path)  # Changed function call
            ...
            for i, (shard_temp_path, tables_in_shard) in enumerate(shard_db_files):
                ...
                compressed_shard_name = f"{shard_file_name_base}.db.gz"  # Changed extension
                compressed_shard_path = output_dir / compressed_shard_name

                compress_gzip(shard_temp_path, compressed_shard_path)  # Changed function call
                ...
                logging.info(
                    f"Created shard {i:03d}: {compressed_shard_name} for tables {tables_in_shard}"
                )
    ...
    def _split_database_by_table(
        self, original_db_path: Path, temp_files_list: list[Path]
        ...
        try:
            ...
            for table_name, estimated_size_bytes in tables_data:
                ...
                if (
                    current_shard_tables
                    and (current_shard_size_bytes + table_compressed_estimate)
            ...
            for i, tables_in_shard in enumerate(shards_tables_map):
                shard_temp_db_path = (
                    self.temp_dir / f"temp_shard_{i:03d}_{original_db_path.stem}.db"
                )
                ...
                try:
                    for table_name in tables_in_shard:
                        ...
                        try:
                            ddl_result = original_conn.execute(
                            ...
                            if ddl_result and ddl_result[0]:
                                ...
                                logging.warning(
                                    f"Could not get DDL for table {table_name}. Attempting simple SELECT *."
                                )
                                # This won't preserve constraints, indexes etc.
                                shard_conn.execute(
                                    f"CREATE TABLE {table_name} AS SELECT * FROM read_parquet('{original_db_path.resolve()}::{table_name}');"
                                )
                                ...
                                original_conn.execute(
                                    f"EXPORT DATABASE '{shard_temp_db_path}' (FORMAT 'parquet', TABLE '{table_name}');"
                                )
                                ...
                                shard_conn.execute(
                                    f"INSERT INTO {table_name} SELECT * FROM {table_name}_df;"
                                )

                        except Exception as e_copy:
                            logging.error(
                                f"Error copying table {table_name} to shard {i}: {e_copy}"
                            )
            ...
            logging.info(
                f"Successfully created {len(shard_file_tuples)} temporary shard database files."
            )
    ...
    def _generate_merge_script(self, shard_infos: list[ShardInfo]) -> str:
        ...
        for i, shard in enumerate(shard_infos):
            ...
            decompressed_shard_file = shard.file.replace(".gz", "")  # Changed extension

            script_lines.append(
                f"-- Processing Shard: {shard.file} (contains tables: {shard.tables}) --"
            )
            script_lines.append(
                f"ATTACH '{decompressed_shard_file}' AS {shard_db_alias} (READ_ONLY);"
            )
            ...
            if shard.tables and shard.tables != ["ALL"]:  # Avoid for single, non-sharded file
                ...
                script_lines.append(
                    f"-- This is a single-file snapshot, tables are already in '{decompressed_shard_file}'."
                )
                script_lines.append("-- No explicit table copy needed if this is the only shard.")
    ...
    def reconstruct_from_shards(
        self, manifest: SnapshotManifest, snapshot_dir: Path, output_db_path: Path
    ) -> None:
        """
        ...
        try:
            ...
            for shard_info in manifest.shards:
                ...
                if not compressed_shard_path.exists():
                    raise FileNotFoundError(
                        f"Shard file {shard_info.file} not found in {snapshot_dir}"
                    )
                ...
                # 2. Decompress shards
                decompressed_name = shard_info.file.replace(".gz", "")  # Changed extension
                decompressed_path = temp_decompress_dir / decompressed_name
                decompress_gzip(compressed_shard_path, decompressed_path)  # Changed function call
            ...
            if not manifest.merge_script and len(decompressed_shard_paths) == 1:
                ...
                logging.info(
                    f"Single shard snapshot. Copied {decompressed_shard_paths[0]} to {output_db_path}"
                )
                ...
                try:
                    ...
                    for (
                        _,
                        ddl_statement,
                    ) in DUCKDB_SCHEMA.items():  # Using DUCKDB_SCHEMA from migration script context
                        try:
                            target_conn.execute(ddl_statement)
                        except Exception as e_ddl:
                            logging.warning(
                                f"Could not pre-create table via DUCKDB_SCHEMA: {e_ddl}"
                            )

                    for i, shard_info in enumerate(manifest.shards):
                        shard_db_alias = f"shard_{i:03d}_db"
                        # Find the corresponding decompressed path
                        current_decompressed_path = next(
                            p
                            for p in decompressed_shard_paths
                            if p.name == shard_info.file.replace(".gz", "")  # Changed extension
                        )

                        target_conn.execute(
                            f"ATTACH '{current_decompressed_path}' AS {shard_db_alias} (READ_ONLY);"
                        )
                        ...
                        if shard_info.tables and shard_info.tables != ["ALL"]:
                            for table_name in shard_info.tables:
                                try:
                                    ...
                                    logging.error(
                                        f"Error copying table {table_name} from {shard_db_alias}: {e_copy_table}"
                                    )
                        elif (
                            shard_info.tables == ["ALL"] and len(manifest.shards) > 1
                        ):  # Should not happen if properly sharded
                            logging.warning(
                                f"Shard {shard_info.file} marked as ALL tables but is part of multi-shard snapshot. Check logic."
                            )
                ...
                raise ValueError(
                    "Cannot reconstruct: No merge script for multiple shards, and not a single shard snapshot."
                )
            ...
            if final_merkle_root != manifest.merkle_root:
                ...
                logging.info(
                    f"Reconstructed database Merkle root matches manifest: {final_merkle_root}"
                )
            ...
            if temp_decompress_dir.exists():
                shutil.rmtree(
                    temp_decompress_dir
                )  # Clean up the whole temp dir for decompressed shards
